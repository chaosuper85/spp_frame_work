 ...
 * @package Cache
 */
abstract class SPP_Cache_Cache {
    /**
     * 是否启用cache
     *
     * @var bool
     ...
     * @var array
     */
    private static $mStorages = array('file' => 'Cache_File','db' => 'Cache_DB','memcache' => 'Cache_Memcache','apc' => 'Cache_Apc','xcache' => 'Cache_Xcache','x' => 'Cache_Xcache' );
    private static $instances = array();
    public static function factory($storage = 'file', $pOptions = array()) {
        if (! $storage instanceof SPP_Cache_Cache) {
            $tClass = '';
    ...
    public static function getInstances() {
        ...
        if (is_null(self::$mStorage)) {
            self::$mStorage = new BaseObjectStorage();
    ...
    public function setOption($key, $value) {
        if (! class_exists('Util_Util', false)) {
            require_once (dirname(__FILE__) . '/Util.php');
        }
        $key = 'm' . SPP_Util_Util::magicName($key);
        $this->$key = $value;
    }
    /**
     * 获得属性的值
     *
     * @param string $key
     *            属性/参数的值，支持形如cahce_dir或mCacheDir二种方式
     * @return mixed
     */
