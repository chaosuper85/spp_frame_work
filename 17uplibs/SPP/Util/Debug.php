...
class SPP_Util_Debug {
    ...
    function &getInstance() {
        if (is_null(self::$_theInstances)) {
            self::$_theInstances = new SPP_Util_Debug();
        }
        return self::$_theInstances;
    }
    static function getDumpInfo() {
        $sql = array();
        $object = array();
        $echo = <<<EOF
...
<head>
    <title>FrameWork Debug Console</title>
{literal}
<style type="text/css">
/* <![CDATA[ */
body, h1, h2, td, th, p {
...
// <![CDATA[
    _framework_debug_console = window.open('','_blank',"width=680,height=600,resizable,scrollbars=yes");
    _framework_debug_console.document.write('$content');
    _framework_debug_console.document.close();
// ]]>
</script>
EOF;
    }
    public static function setDebugInfo($pInfo) {
        list($msec, $sec) = explode(" ", microtime());
        array_push(self::$_info, array_merge(array(date("m-d H:i:j") . substr($msec, 1) ), $pInfo));
    }
     ...
     * @return string
     */
    static function smartyModifierEscape($string, $esc_type = 'html', $char_set = 'ISO-8859-1') {
        switch($esc_type) {
            case 'html' :
