<?php
/**
 * Table <==> Class 映射基础类库，自动提供基本的插入，更新，删除，搜索功能，根据表结构自动生成相应 SQL 代码
 */
class SPP_BaseTable extends SPP_Object implements ArrayAccess, IteratorAggregate {
    /**
     * 当前对象对应表信息
     *
     * @var array
    ...
    public function __construct($pTableName, $pDBConfig = 0) {
        parent::__construct();
        $this->SqlCommand = SPP_SqlCommand::getInstance($pDBConfig);
    ...
    private function prepareMapHash() {
        // 处理表字段
        $tFileCache = new SPP_Cache_FileCache('fields' . $this->mTableHash['name']);
        if ($tFileCache->check_cache_status()) {
            $this->mTableFieldHash = unserialize($tFileCache->get_cache_content());
        } else {
            $this->mTableFieldHash = $this->SqlCommand->getTableFieldHash($this->mTableHash['name']);
            if (count($this->mTableFieldHash) > 0) {
                if (SPP_Config_Config::getMode() == "online") {
                    // 如果系统运行在online模式下,则记录缓存,默认是online模式
                    $tFileCache->put_cache_content(serialize($this->mTableFieldHash));
                }
            } else {
        ...
        for($i = 0; $i < count($this->mTableFieldHash); $i ++) {
            $member_name = "m" . SPP_Util_Util::magicName($keys[$i]);
            $this->mMapHash[$keys[$i]] = &$this->$member_name;
        }
        
        // 处理主键
        $tFileCache = new SPP_Cache_FileCache('primaryKey' . $this->mTableHash['name']);
        if ($tFileCache->check_cache_status()) {
            $this->mTableHash['key'] = unserialize($tFileCache->get_cache_content());
        } else {
            $this->mTableHash['key'] = $this->SqlCommand->getTablePrimeKey($this->mTableHash['name']);
            if (SPP_Config_Config::getMode() == "online") {
                // 如果系统运行在online模式下,则记录缓存,默认是online模式
                $tFileCache->put_cache_content(serialize($this->mTableHash['key']));
            }
        }
    }
        ...
        foreach($ObjectSource->mMapHash as $key => $value) {
            if (isset($ObjectSource->mMapHash[$key])) {
                if ('int' != $ObjectSource->mTableFieldHash[$key]['type'] && 'bigint' != $ObjectSource->mTableFieldHash[$key]['type']) {
                    $array[$key] = $this->SqlCommand->escapeString($value);
                } else {
    ...
    public function _select($pLimit = 1, $pOrder = NULL) {
        $tSmartCode = ($pLimit > 1) ? 1 : 0;
        $tResult = $this->_list($pLimit, 1, $pOrder, $tSmartCode);
        if (count($tResult) > 0) {
            return $this->_fill($tResult[0]);
        }
        return false;
    }
     ...
     * @return bool
     */
    public function _fetchOne($pLimit = 1, $pOrder = NULL, $pColumns = '*') {
        $tSmartCode = ($pLimit > 1) ? 1 : 0;
        $tResult = $this->_list($pLimit, 1, $pOrder, $tSmartCode, $pColumns);
        if (count($tResult) > 0) {
            return $tResult[0];
        }
        return false;
    ...
    function _list($pPage = 0, $pPageSize = 10, $pOrder = null, $pSmartCode = 1, $pColumns = '*', $pGroupBy = '') {
        if ($pPage != 0 && $pPageSize != 1) {
            ...
            $this->mPagination = new SPP_Pagination($pPage, $pPageSize, $count);
        }
